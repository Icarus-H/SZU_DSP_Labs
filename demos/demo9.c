...
*   我不知道除了0之外的通道都是干嘛的，拿到的数值也奇奇怪怪的，目前就知道1，2，7（7还不好用，拿到的数值会跳）是能用的，其他的没试过，8和9是不行的
**********************************************************************/
/********************************************************************
// 功能描述:
//      用跳线帽将J9插件的1,2引脚短接
...
//#include "demo7.h"
#include "demo9.h"
#define stuNum 0x07 // 学号末位，用作第二通道
...
//   AdcRegs.ADCCHSELSEQ2.bit.CONV06 = 0x0;   //ADC通道选择ADCIN6
   ...
   AdcRegs.ADCMAXCONV.all = 0x0002;         // 最多采样 AdcRegs.ADCMAXCONV.all+1 个

   AdcRegs.ADCTRL2.bit.INT_ENA_SEQ1 = 0;    // 真牛逼，我得把中断关了再来才能做中断实验，感觉是下面的中断函数喧宾夺主把ADC中断了
